...
<head>
    ...
    <title>Invoice Multiple</title>
</head>
<body>
    @php $i = count($data); @endphp
        ...
        <div class="w-full h-auto mr-4 bg-white p-2 sm:p-8 rounded-xl">
            <div class="flex items-center">
                <table class="no-border">
                    <tr>
                        <td>INVOICE</td>
                        <td><div class="{{$pembayaran[$key]['sisa_tagihan'] > 0 ? 'belumlunas' : 'lunas'}}">{{$pembayaran[$key]['status_lunas']}}</div></td>
                    </tr>
                </table>
            </div>
                ...
                <table class="no-border">
                    <tr>
                        <td>
                            ...
                            <div class="text-[18px] font-robotomedium">{{$key}}</div>
                        </td>
                        <td>
                            <div class="text-xs font-robotoregular mt-2">Tanggal</div>
                            <div class="text-[18px] font-robotomedium">{{$tanggal_pembuatan[$key]}}</div>
                        </td>
                        <td>
                            <div class="text-xs font-robotoregular mt-2">Nama Agen</div>
                            <div class="text-[18px] font-robotomedium">{{$agen[$key][0]['id'] ? $agen[$key][0]['nama_agen'] : '-'}}</div>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <div class="text-xs font-robotoregular mt-2">Dermaga Asal</div>
                            <div class="text-[18px] font-robotomedium">{{$penumpang[$key][0]['dermaga_awal']}}</div>
                        </td>
                        <td>
                            <div class="text-xs font-robotoregular mt-2">Dermaga Tujuan</div>
                            <div class="text-[18px] font-robotomedium">{{$penumpang[$key][0]['dermaga_tujuan']}}</div>
                        </td>
                        <td>
                            <div class="text-xs font-robotoregular mt-2">Nama Kapal</div>
                            <div class="text-[18px] font-robotomedium">{{$penumpang[$key][0]['nama_kapal']}}</div>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                            <div class="text-xs font-robotoregular mt-2">Waktu Keberangkatan</div>
                            <div class="text-[18px] font-robotomedium">{{$penumpang[$key][0]['waktu_berangkat']}}</div>
                        </td>
                        <td></td>
                    </tr>
                </table>
            ...
            <table class="penumpang">
                ...
                <tbody>
                    <?php foreach ($penumpang[$key] as $k => $p) { ?>
                    <tr class="text-sm font-robotoregular">
                        <td class="pl-4 py-2">{{$k+1}}</td>
            ...
            <table class="penumpang">
                ...
                <tbody>
                    <tr class="text-sm font-robotoregular undefined undefined">
                        ...
                        <td class="py-2">Tiket 
                            <span class="font-robotoregular text-primary">{{$data[$key][0]['is_pp'] == 'true' ? ' - (Pulang Pergi)' : ''}}</span>
                        </td>
                        <td class="py-2">{{$data[$key][0]['jenis_penumpang']}}</td>
                        <td class="py-2">{{$data[$key][0]['jumlah_tiket']}}</td>
                        <td class="py-2">Rp. {{ number_format($data[$key][0]['harga_tiket'], 0, '', '.') }}</td>
                        <td class="py-2">Rp. {{ number_format($data[$key][0]['diskon_agen'], 0, '', '.') }}</td>
                        <td class="py-2">Rp. {{ number_format($data[$key][0]['subtotal_tiket'] - $data[$key][0]['subtotal_diskon'], 0, '', '.') }}</td>
                    </tr>
                    <tr class="text-sm font-robotoregular bg-[#F0F0F0] dark:bg-slate-500 undefined">
                        ...
                        <td class="py-2">-</td>
                        <td class="py-2">{{$data[$key][0]['jumlah_service']}}</td>
                        <td class="py-2">Rp. {{ number_format($data[$key][0]['harga_service'], 0, '', '.') }}</td>
                        <td class="py-2">-</td>
                        <td class="py-2">Rp. {{ number_format($data[$key][0]['subtotal_service'], 0, '', '.') }}</td>
                    </tr>
                    <tr class="text-sm font-robotoregular bg-[#F0F0F0] dark:bg-slate-500 undefined">
                        ...
                        if (count($cl) == 1) {
                        @endphp
                        <td class="py-2">Rp. {{ number_format($cl[0]->jumlah, 0, '', '.') }}</td>
                        <td class="py-2">-</td>
                        <td class="py-2">Rp. {{ number_format($cl[0]->jumlah, 0, '', '.') }}</td>
                        @php
                        } else {
                    ...
                    <tr class="text-sm font-robotoregular border border-[black] border-x-0">
                        ...
                        @if (count($cl) == 1)
                        <td colspan="2" class="text-lg py-1 font-robotobold">Rp. {{ number_format($data[$key][0]['subtotal']+(double) $cl[0]->jumlah, 0, '', '.') }}</td>
                        @else
                        <td colspan="2" class="text-lg py-1 font-robotobold">Rp. {{ number_format($data[$key][0]['subtotal'], 0, '', '.') }}</td>
                        @endif
                    </tr>
                </tbody>
            </table>
